...
uint8 *get_option(struct dhcpMessage *packet, int code)
{
    ...
    while (!done) {
        if (i >= length) {
            LOG_WARN("bogus packet, option fields too long.\n");
            return NULL;
        }
...
int add_option_string(uint8 *optionptr, uint8 *string)
{
    ...
    {
        LOG_ERRO("option 0x%02x did not fit into the packet!\n", string[OPT_CODE]);
        return 0;
    }

    LOG_VERB("adding option 0x%02x\n", string[OPT_CODE]);
...
int add_simple_option(uint8 *optionptr, uint8 code, uint32 data)
{
    ...
    if (!length) {
        LOG_ERRO("Could not add option 0x%02x\n", code);
        return 0;
    }
